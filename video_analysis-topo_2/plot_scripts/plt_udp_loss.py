...
def main():
	...
	file_name = 'all_udp_loss.csv'

	plt_title = 'UDP Loss (Case %s)'
	...
	for row in csv_file:
		...
		# dividing each element by div_val
		div_val = 1000

		new_fl_row = [x/float(div_val) for x in new_fl_row]
	...
	for i in xrange(2):
		...
		loss_hd = csv_data_np[plt_num + 4*i][0::2] # at even position 0 2 4
		loss_sd = csv_data_np[plt_num + 4*i][1::2] # at odd position 1 3 5
	...
	plt.ylabel('Lost Packets (x1000)', fontsize = plt_font_size_axis)
